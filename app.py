...
EXTENDED_PLAY_DB = {
    ...
    (7, 9): [
        ...
        ("인지", "🔍 손수건 속 장난감 찾기", "물건을 손수건으로 살짝 가려 대상영속성(보이지 않아도 존재함)을 익힙니다."),
        ("대근육", "🎾 굴러가는 공 잡으러 기어가기", "배밀이나 기어가기를 유도하기 위해 아이가 좋아하는 공이나 장난감을 굴려줍니다."),
        ("언어", "🗣️ 동물 소리 & 까꿍 대화", "엄마 아빠의 입 모양을 보여주며 음성 언어 모방을 유도합니다."),
        ("소근육", "🥣 핑거푸드(떡뻥) 집어먹기", "엄지와 검지로 작은 과자 조각을 직접 집어먹으며 핀서 그립(Pincer grasp)을 연습합니다.")
    ],
    (10, 12): [
        ("소근육/인지", "🧱 컵 쌓기 & 와르르 무너뜨리기", "컵을 높이 쌓아주고 아이가 손으로 무너뜨리며 인과관계와 성취감을 경험합니다."),
        ("분류/공간", "📦 상자 속에 공 넣고 쏟기", "작은 공이나 블록을 바구니에 넣고 다시 쏟는 반복 탐색 놀이를 진행합니다."),
        ("대근육", "🧍 소파 잡고 서서 꽃게걸음(크루징)", "소파나 안전 가드를 잡고 옆으로 걸어보는 균형 잡기 연습을 합니다."),
        ("소근육", "📄 부드러운 종이 찢기 놀이", "전단지나 신문지를 손으로 찢고 구기며 스트레스 해소 및 악력을 기릅니다."),
        ("언어/사회성", "👋 안녕! 바이바이 손인사", "외출 시나 헤어질 때 손을 흔드는 사회적 상호작용 제스처를 배웁니다.")
    ],
    (13, 18): [
        ("소근육/표현", "🖍️ 안전 무독성 크레파스 끄적이기", "큰 전지 위에 자유롭게 선을 그으며 손목 조절력과 표현력을 기릅니다."),
        ("인지/공간", "🧩 꼭지 퍼즐 & 도형 맞추기", "동그라미, 세모, 네모 기본 도형을 제자리에 맞추는 공간지각력을 키웁니다."),
        ("대근육", "⚽ 거실 축구 & 공 굴려 주고받기", "발로 공을 차거나 손으로 굴리며 신체 균형 감각과 대근육을 발달시킵니다."),
        ("생활습관", "🧸 숟가락으로 인형 밥 먹이기", "소꿉놀이 도구로 인형에게 음식을 먹여주는 흉내 놀이를 통해 공감 능력을 키웁니다."),
        ("감각", "🌾 미역/쌀 튀밥 오감 촉감 놀이", "불린 미역이나 쌀 튀밥을 매트 위에서 만지고 으깨는 오감 자극을 줍니다.")
    ],
    (19, 24): [
        ("역할놀이", "🛒 마트 장보기 & 카트 밀기", "모형 과일과 장바구니를 활용해 물건을 사고파는 초기 상징 놀이를 진행합니다."),
        ("소근육", "🧵 굵은 빨대 실에 꿰기 놀이", "빨대를 2cm로 잘라 끈이나 털실에 꿰며 고도의 집중력과 눈-손 협응을 기릅니다."),
        ("대근육", "🪜 베개 징검다리 건너기", "바닥에 베개나 쿠션을 놓고 밟으며 중심을 잡고 건너는 장애물 놀이를 합니다."),
        ("언어", "📖 그림책 장면 속 사물 가리키기", "'멍멍이는 어디 있지?' 질문에 맞춰 손가락으로 가리키는 수용 언어를 확장합니다."),
        ("사회성", "🧼 손 씻기 & 거품 비누 거품 놀이", "거품을 만들고 손을 비비며 청결 습관과 촉각적 재미를 함께 느낍니다.")
    ],
    (25, 36): [
        ("인지/창의", "🧱 레고 듀플로 / 블록 구조물 만들기", "집, 터널, 기차역 등 상상하는 형태를 블록으로 조립하며 창의력을 발휘합니다."),
        ("신체", "🛑 무궁화 꽃이 피었습니다 (얼음 놀이)", "정지 신호에 멈추는 연습을 통해 충동 억제와 신체 조절력을 단련합니다."),
        ("언어", "🎭 역할극 (병원놀이 / 미용실 놀이)", "의사와 환자 역할을 번갈아 하며 사회적 대화 규칙과 어휘력을 확장합니다."),
        ("미술", "🎨 플레이도우 점토로 피자 만들기", "도우를 밀대로 밀고 모형 칼로 자르며 소근육 정교성을 향상합니다."),
        ("자연탐구", "🍂 나뭇잎 주워 색깔별 분류하기", "공원 산책 시 낙엽, 돌멩이를 모아 크기나 색상별로 분류해 봅니다.")
    ],
    (37, 48): [
        ("소근육/가위질", "✂️ 안전 가위로 종이 선 따라 오리기", "직선, 곡선을 가위로 오려보며 양손 협응력과 주의집중력을 키웁니다."),
        ("규칙/게임", "🎲 주사위 굴려 보드게임 칸 이동", "단순한 규칙을 지키고 차례를 기다리는 사회적 규칙을 학습합니다."),
        ("신체", "🚲 세발자전거 페달 밟기", "페달을 밟아 앞으로 나아가는 협응 대근육 운동을 진행합니다."),
        ("과학/인지", "🧊 얼음 속에 갇힌 장난감 구출", "얼음이 녹는 과정을 관찰하고 따뜻한 물이나 소금으로 녹여봅니다."),
        ("음악", "🥁 냄비 뚜껑 & 컵 난타 연주", "주방 도구로 다양한 소리와 리듬을 만들어보는 청각 음악 놀이입니다.")
    ],
    (49, 72): [
        ("논리/창의", "🗺️ 보물 지도 그리고 숨긴 물건 찾기", "집안 구조를 지도로 간단히 그리고 힌트를 주어 추리 능력을 발달시킵니다."),
        ("언어/문해", "📝 끝말잇기 & 초성 퀴즈", "소리와 글자의 관계를 익히고 어휘 연상 속도를 높입니다."),
        ("신체/스포츠", "🏸 미니 배드민턴 & 줄넘기 기초", "도구를 활용한 스포츠를 통해 순발력과 체력을 증진합니다."),
        ("과학/실험", "🌋 베이킹소다 식초 화산 폭발", "탄산가스 발생 화학반응을 직접 눈으로 관찰하는 기초 과학 탐구입니다."),
        ("정서/협동", "🧩 50~100피스 직소 퍼즐 완성하기", "목표를 끝까지 완수하는 끈기와 공간 구성력을 기릅니다.")
    ]
}
